...
func getEncoder() zapcore.Encoder {
	...
	encoderConfig := zapcore.EncoderConfig{
		...
		LevelKey:       "level",
		NameKey:        "logger",
		CallerKey:      "caller", // 代码调用，如 paginator/paginator.go:168
		FunctionKey:    zapcore.OmitKey,
		MessageKey:     "message",
		StacktraceKey:  "stacktrace",
		LineEnding:     zapcore.DefaultLineEnding,      // 每行日志的末尾添加 "\n"
		EncodeLevel:    zapcore.CapitalLevelEncoder,    // 日志级别名称大写，如 ERROR,INFO
		EncodeTime:     customerTimeEncoder,            // 时间格式，我们自定义为 2006-01-02 15:04:05
	...
	if app.IsLocal() {
		...
		// 本地内置的 Console 解码器（支持 stacktrace 换行）
...
// customerTimeEncoder 自定义有好的时间格式
func customerTimeEncoder(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
	enc.AppendString(t.Format("2006-01-02 13:04:05"))
}

// getLogWriter 日志记录介质。Yfthhub 中使用了两种介质，os.Stdout 和文件
