...
// ConnectRedis 链接 redis 数据库，设置全局的 Redis 对象
...
// 当参数有 2 个时，第一个参数作为 key ，第二个参数为要增加的值 int64 类型。
func (rds RedisClient) Increment(parameters ...interface{}) bool {
	...
	default:
		logger.DebugString("Redis", "Increment", "参数过多")
		return false
	}
	return true
}
...
func (rds RedisClient) Decrement(parameters ...interface{}) bool {
	...
	default:
		logger.DebugString("Redis", "Decrement", "参数过多")
		return false
	}
	return true
}
